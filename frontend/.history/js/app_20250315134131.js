...
const contractAddress = "0x5A8B4a39eCFE0191C930b7Ebe85Eafa1A219cB26";
const contractABI = [
  {
    inputs: [
      {
  ...
  try {
    ...
    document
      ...
      .addEventListener("submit", async function (event) {
        ...
        await initializeBooking(room.roomId, formattedStartTime, minutes);
      });
  } catch (error) {
...
//   try {
//     if (!accounts || accounts.length === 0) {
//       throw new Error("Không có tài khoản nào có sẵn.");
//     }

//     //const accountBytes32 = web3.utils.padLeft(account, 64);
//     const balance = await web3.eth.getBalance(accounts[0].address);
//     console.log("Tài khoản: " + accounts[0].address);
//     console.log(
//       "Số dư tài khoản (tBNB): " + web3.utils.fromWei(balance, "ether")
//     );
...
async function initializeBooking() {
  // Lấy thông tin từ các input trên form
  const startTime = document.getElementById("startTime").value;
  const minutes = document.getElementById("minutes").value;

  if (!startTime || !minutes) {
    alert("Vui lòng nhập đầy đủ thông tin về thời gian bắt đầu và thời gian thuê!");
    return;
  }

  try {
    // Địa chỉ tài khoản gửi giao dịch
    const fromAddress = accounts[0].address;

    // Lấy giá gas hiện tại từ mạng blockchain
    const gasPrice = await web3.eth.getGasPrice();

    // Tính toán gas limit cho giao dịch này
    const gasLimit = await contract.methods
      .initializeBooking(roomId, startTime, minutes) // Sử dụng các tham số đã nhập từ form
      .estimateGas({ from: fromAddress });

    // Tạo đối tượng giao dịch
    const tx = {
      to: contractAddress,
      data: contract.methods.initializeBooking(roomId, startTime, minutes).encodeABI(),
      gas: gasLimit,
      gasPrice: gasPrice,
      from: fromAddress,
    };
    ...
    // In ra kết quả và thông báo thành công
