const contractAddress = "0x50F70881c5e5eD764F922aAd83A3131b71204627";
const contractABI = [
  {
    anonymous: false,
    inputs: [
...
async function renderRooms() {
  try {
    ...
    roomListContainer.innerHTML = ""; // Xóa danh sách phòng hiện tại
    ...
    rooms.forEach((room) => {
      ...
      roomElement.innerHTML = `
        <div>
          ...
          Length: ${room.roomLength} meters<br />
          Price per Hour: $${room.pricePerHour}<br />
          Available: ${room.isAvailable ? "Yes" : "No"}<br />
          Seats Initialized: ${room.seatsInitialized ? "Yes" : "No"}<br />
        </div>
        <hr>
      `;
...
// Tải danh sách phòng khi trang được tải
renderRooms();
