...
async function fetchAllBookings() {
  ...
  // Call the getAllBookings function from your smart contract
  try {
    ...
    bookingListContainer.innerHTML = ""; // Clear any previous content
...
async function cancelBooking(bookingId) {
  ...
  try {
    await contract.methods.cancelBooking(bookingId).send({ from: accounts[0] });
    alert("Booking đã được hủy.");
    fetchAllBookings(); // Refresh the booking list after cancellation
  } catch (error) {
    alert("Lỗi khi hủy booking: " + error.message);
  }
}

// Call the function to fetch and display all bookings
fetchAllBookings();
