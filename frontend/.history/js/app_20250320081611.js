...
async function initializeBooking(roomId, startTimestamp, minutes) {
  try {
    ...
    // Convert the start time and end time of the new booking
    const newEndTimestamp = startTimestamp + minutes * 60;

    // Check for overlapping bookings for the specified room
    for (let i = 0; i < allBookings.length; i++) {
      const booking = allBookings[i];

      // Check if this booking is for the same room and is not cancelled
      if (booking.roomId == roomId && booking.isCancel === false) {
        const bookingStartTime = booking.startTime;
        const bookingEndTime = booking.endTime;

        // Check if the new booking's time overlaps with any existing booking
        if (
          startTimestamp < bookingEndTime &&
          newEndTimestamp > bookingStartTime
        ) {
          alert("Thời gian bạn chọn bị trùng với một booking khác.");
          return;
        }
      }
    }

    // Estimate gas for the new booking
    const gasLimit = await contract.methods
...
async function cancelBooking(bookingId) {
  try {
    ...
    const tx = {
      to: contractAddress, 
      data: contract.methods.cancelBooking(bookingId).encodeABI(),
      gas: gasLimit, 
      gasPrice: gasPrice, 
      from: fromAddress, 
    };
