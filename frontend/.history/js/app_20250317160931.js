...
const contractAddress = "0x5A4A8549552e76D0F9Cd7AEfD2Cbb3f29675185B";
const contractABI = [
  {
    inputs: [
      {
...
async function fetchBookings() {
  ...
  const bookingsCount = await bookingContract.methods.nextBookingId().call(); // assuming nextBookingId holds the count
  const bookingListContainer = document.getElementById("bookingList");
  bookingListContainer.innerHTML = ""; // Clear any previous bookings

  // Loop through each booking and display its details
  for (let i = 1; i < bookingsCount; i++) {
    const booking = await bookingContract.methods.bookings(i).call();

    // Format the booking start and end times
    const startDate = new Date(booking.startTime * 1000);
    const endDate = new Date(booking.endTime * 1000);

    const roomType = await bookingContract.methods.rooms(booking.roomId).call(); // Assuming `rooms` is a function that fetches room details by id

    const bookingElement = document.createElement("div");
    bookingElement.classList.add("booking-item");

    bookingElement.innerHTML = `
            <p><strong>Phòng:</strong> ${roomType.name}</p>
            <p><strong>Loại phòng:</strong> ${roomType.roomType}</p>
            ...
            <p><strong>Trạng thái:</strong> ${
              booking.isCompleted ? "Hoàn thành" : "Chưa hoàn thành"
            }</p>
