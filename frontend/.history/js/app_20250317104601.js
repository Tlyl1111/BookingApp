...
function showInitializeBookingForm(room) {
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      event.preventDefault(); // Ngừng sự kiện submit mặc định

      const startTime = document.getElementById("startTime").value; // Thời gian bắt đầu
      const minutes = document.getElementById("minutes").value; // Thời gian
      ...
      // Chuyển thời gian bắt đầu từ định dạng datetime-local sang timestamp
      const startTimestamp = new Date(startTime).getTime() / 1000; // Chuyển đổi thành giây

      const roomId = room.roomId; // ID của phòng

      // Gọi hàm initializeBooking từ hợp đồng Solidity
