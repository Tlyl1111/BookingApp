...
async function initializeBooking(roomId, startTime, minutes) {
  try {
    ...
    // In ra tài khoản và số dư trước khi gửi giao dịch
    const balance = await web3.eth.getBalance(accounts[0].address);
    console.log("Tài khoản: " + accounts[0].address);
    console.log(
      "Số dư tài khoản (BNB): " + web3.utils.fromWei(balance, "ether")
    );

    const result = await contract.methods
      .initializeBooking(roomId, startTime, minutes)
      .send({
        from: accounts[0].address, // Sử dụng tài khoản đầu tiên từ ví
      });
