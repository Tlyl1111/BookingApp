...
function showInitializeBookingForm(room) {
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      const startTime = document.getElementById("startTime").value; // Get the start time from the input field
      const minutes = document.getElementById("minutes").value; // Get the duration in minutes

      // Convert startTime to YYYYMMDDHHMMSS format
      const formattedStartTime = formatDateToYYYYMMDDHHMMSS(startTime);

      initializeBooking(room.roomId, formattedStartTime, minutes);
    });

  // Function to format the date to YYYYMMDDHHMMSS
  function formatDateToYYYYMMDDHHMMSS(dateStr) {
    // Convert the date string to a JavaScript Date object
    const date = new Date(dateStr);

    // Get the individual components (year, month, day, etc.)
