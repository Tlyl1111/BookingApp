...
function handleRoomClick(room) {
  console.log("Room ID clicked: ", room.roomId); 
  ...
  try {
    ...
    document
      ...
      .addEventListener("submit", async function (event) {
        ...
        let roomId = parseInt(room.roomId, 10); // Sử dụng parseInt để chuyển sang số nguyên

        if (isNaN(roomId)) {
          console.error("Invalid Room ID:", roomId); // Kiểm tra nếu roomId không hợp lệ
          return;
        }
