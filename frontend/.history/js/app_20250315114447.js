...
async function checkBalance() {
  const balance = await web3.eth.getBalance(accounts[0].address); // Lấy số dư của tài khoản đầu tiên
  console.log(
...
async function initializeBooking(roomId, startTime, minutes) {
  try {
    ...
    const accountBytes32 = web3.utils.padLeft(accounts[0].address, 64); // Dạng bytes32 có độ dài 64 ký tự

    const result = await contract.methods
      .initializeBooking(roomId, startTime, minutes)
      .send({
        from: accountBytes32, // Sử dụng accountBytes32
      });
